...
export const getReservations = (): Promise<IReservation[]> => {
  return new Promise((resolve, reject) => {
    ...
    setTimeout(() => {
      const reservations: IReservation[] = ListingData.results
        .slice(8, 10)
        .map((item) => {
          return {
            id: item.id,
...
export const getReservationById = (id: string): Promise<IReservation> => {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      const reservation: IReservation = ListingData.results
        .filter((x) => x.id === id.toString())
        .map((item) => {
          return {
            id: item.id,
            name: item.name,
  ...
  return new Promise((resolve, reject) => {
    axios
      ...
      .catch((err) => {
        ...
        console.log(err.response.status); // Log the HTTP status code
